...
class MarsAjaxForms {
    ...
    /**
     * Enables the AJAX form actions for the given element
     * @protected
     * @param {string|object} element The element over which ajax form actions will be enabled
     */
    enable (element) {
    ...
    async handleSubmit(element, onSuccess = null, onError = null) {
        if (onSuccess === null) {
            onSuccess = (data, form, reset) => this.handleSubmitData(data, form, reset);
        }
        ...
        element.addEventListener('click', async (event) => {
            ...
            let reset = true;
            if (element.dataset[this.datasets.reset]) {
                if (element.dataset[this.datasets.reset] === '0' || element.dataset[this.datasets.reset].toLowerCase() === 'false') {
                    reset = false;
                }
            }
     ...
     * @param {boolean} reset Indicates whether the form should be reset after the submit action
     */
    handleSubmitData (data, form, reset = true) {
        if (reset) {
            form.reset();
        }
...
mars.define('ajaxForms', () => new MarsAjaxForms).enable(element => mars.ajaxForms.enable(element));